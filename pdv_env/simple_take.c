...
#endif
{
    ...
    *bmpfname = '\0';
    ...
    printf("reading %d image%s from '%s'\nwidth %d height %d depth %d\n",
            loops, loops == 1 ? "" : "s", cameratype, width, height, depth,gain);
...
/*Function to save fits file of type double, requires pointer to the data that needs to be saved, no of dimensions, length of each dimension
and a file name, file name shouldn't have fits extenstion*/
int savefitsfile_double(double *datatosave, long numberofdimensions, long *dimensionvalues, char *fitsfilename) 
{
    ...
    /*Origin Pixel, may need to be changed later*/
    fpixel[0] = fpixel[1] = 1;
    ...
    fits_create_img(fitsfilepointer, FLOAT_IMG, numberofdimensions, dimensionvalues, &status);
    if (status != 0) {
        fits_report_error(stderr, status);
        return status;
    }

    fits_write_pix(fitsfilepointer, TDOUBLE, fpixel, nelements, datatosave, &status);
    if (status != 0) {
        fits_report_error(stderr, status);
        return status;
    }
...
save_image(u_char * image_p, int s_width, int s_height, int s_depth, char *tmpname, int count)
{
    ...
    long dimvalues[3] = {s_width, s_height, s_depth};
    ...
    switch (s_db)
    {
        case 1:
            ...
            /*Write to FITS with 3 dims, length width
            and bit depth */
        ...
        case 3:
            ...
            printf("%d writing %dx%dx%d bmp file to %s\n",
                   s_width, s_height, s_depth, fname);
            break;
...
{
    puts(errmsg);
    printf("%s: simple example program that acquires images testtest from an\n", progname);
