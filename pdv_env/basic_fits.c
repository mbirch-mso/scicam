...
#endif
{
    ...
    *fitsfname = '\0';
    ...
    if (*fitsfname){
        save_image(image_p, width, height, depth, fitsfname);
    }
    puts("");
...
int savefitsfile_double(u_char *datatosave, int numberofdimensions, long *dimensionvalues, char *fitsfilename) 
{
    ...
    static void
save_image(u_char *image_p, int s_width, int s_height, int s_depth, char *tmpname)
{
    ...
    printf("%d writing %dx%dx%d FITS file to %s\n",
                   success, s_width, s_height, s_depth, tmpname);
}
