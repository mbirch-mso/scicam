...
COMMON_FILES='
    ...
    pdv_qtlib/EdtQtGraphicsItem.cpp
    pdv_qtlib/EdtQtGraphicsItem.h
    pdv_qtlib/contrastwidget.cpp
    pdv_qtlib/contrastwidget.h
    pdv_qtlib/contrastwidget.ui
    pdv_qtlib/HelpDlg.cpp
    pdv_qtlib/HelpDlg.h
    pdv_qtlib/HelpDlg.ui
    pdv_qtlib/Resources/checkbox.png
    pdv_qtlib/Resources/invert.png
    pdv_qtlib/Resources/pixel_counter.png
    pdv_qtlib/Resources/serial1.png
    pdv_qtlib/Resources/zoom.png
...
WINDOWS_FILES='
    ...
    pdvshow/EdtImageFrame.cpp
    pdvshow/EdtImageFrame.h
    pdvshow/EdtImageWin.cpp
    pdvshow/EdtImageWin.h
    pdvshow/EdtLiveFrame.cpp
    pdvshow/EdtLiveFrame.h
    pdvshow/resource.h
    pdvshow/stdafx.cpp
    pdvshow/stdafx.h
    pdvshow/res/bmp00002.bmp
    pdvshow/res/crop.cur
    pdvshow/res/edtcamdo2.ico
    pdvshow/res/here.cur
    pdvshow/res/mainfram.bmp
    pdvshow/res/pdvshow.ico
    pdvshow/res/pdvshow.rc2
    pdvshow/res/pdvshowdoc.ico
    pdvshow/res/toolbar.bmp
    ...
    wbin/amd64/QtCore4.dll
    wbin/x86/QtCore4.dll
...
#Create shortcut: C:\Users\Public\Desktop\pdvshow.lnk
...
if [[ "$OS" != "Windows_NT" && "$CENTOS_5" == "true" ]] ; then
    echoit "vlviewer build: CentOS 5 is too old to support qt4... SKIP"
else
    if [ "$MAKE" != "" ] ; then
        if [ ! -f $VLVIEWER ] ; then
        echoit_n "Building vlviewer... "
        $MAKE vlviewer >> $LOG 2>&1
        if [ $? -ne 0 ]; then
            echoit "FAIL"
            EXIT_STATUS=1
        else
...
if [ "$MAKE" != "" ] ; then
    ...
    if [ "$OS" == "Windows_NT" ] ; then
      $MAKE pdvshow >> $LOG 2>&1
      MAKEPDVSHOW_STATUS=$?
      $MAKE clseredt >> $LOG 2>&1
      MAKECLSER_STATUS=$?
    fi

    if [[ $MAKEALL_STATUS -ne 0 || $MAKEDISP_STATUS -ne 0 || $MAKEPDVSHOW_STATUS -ne 0 || $MAKEVLV_STATUS -ne 0 ]] ; then
        echoit "FAIL"
