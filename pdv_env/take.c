        ...
        int s_width, int s_height, int s_depth)
{
    ...
    {
        case 1:
            take_printf("writing %dx%dx%d%d bmp file to %s\n",
...
#endif
{
    ...
    int     s_width, s_height, s_depth, s_size, s_db, s_gain;
    ...
    gain = s_gain = pdv_get_gain(pdv_p);
     ...
     */
    {
        ...
        if (dmasize != imagesize)
            take_printf("reading image from %s\nwidth %d height %d depth %d gain %d %s total bytes %d (DMA %d)\n",
            ...
            take_printf("reading image from %s\nwidth %d height %d depth %d gain %d %s total bytes %d\n",
                    pdv_get_cameratype(pdv_p),
                    pdv_get_width(pdv_p),
                    pdv_get_height(pdv_p),
                    pdv_get_depth(pdv_p),
    ...
    for (lc = 0; lc < loops && !GetOut; lc++)
    {
        ...
        if (*rawfname)
        {
            ...
            take_printf("writing %dx%dx%d%d raw file to %s (actual size %d)\n",
                    s_width, s_height, s_depth, s_gain, tmpname, s_size);
        ...
        else take_printf("%d images %d timeouts %d overruns %gain", done, timeouts, overruns, gain);
...
usage(char *progname, char *errmsg, int exitval)
{
    ...
    printf("TESTING COMPILATION OF TAKE\n");
