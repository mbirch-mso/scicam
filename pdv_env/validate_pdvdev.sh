...
fn_validate_ch2()
{
    ...
    IMAGE_VLVRAW=${TEMPDIR}/validate_pdvdev_${FGUNIT}_vlvraw.raw
    ...
    if [ ! -f ./$VLVIEWER ] ; then
        echoit "vlviewer tests: No ./$VLVIEWER and no make${POSTREBUILD}... SKIP"
    else
        # grab a raw image with vlviewer, check the exit value and image checksum
        VLVRAW_SUMERROR=0
        VLVRAW_MSG="none"

        LOCKFILE=/tmp/pdv${FGUNIT}_2.lck
        RET_VLVRAW=0
        if [ -f $LOCKFILE ] ; then
            VLVRAW_MSG="Lock file $LOCKFILE found! If vlviewer isn't already running, delete the .lck file."
            RET=1
        elif [[ $(uname -o | grep -c -i cygwin) != 1 && "$DISPLAY" == "" ]] ; then
            VLVRAW_MSG="Can't open display (for vlviewer) -- Xserver needs to be running."
            RET=1
        else
            $TIMEOUT 5 ./$VLVIEWER -c 2 -u $FGUNIT -f $IMAGE_VLVRAW --init ./${CONFIG} -q >/dev/null 2>&1
            RET_VLVRAW=$?
            CHECKSUM="`cksum $IMAGE_VLVRAW | cut -d' ' -f1`"
            if [ $RET_VLVRAW -ne 0 ] ; then
                VLVRAW_MSG="<$CHECKSUM>, expected <$CHECKSUMS>"
                RET=1
            else
                grep "CH2_GENDATA_CKSUMS" $CONFIG | grep -wq "$GOT_CKSUM"
                if [ $? -ne 0 ] ; then
                    VLVRAW_SUMERROR=1
                    VLVRAW_MSG="take -J -c 2 -u $FGUNIT -f $IMAGE_VLVRAW, got <$GOT_CKSUM>, expected <$EXPECTED_CKSUMS>"
                fi
            fi

            if [ $VLVRAW_SUMERROR -ne 0 ] ; then
                RET=1
            fi
        fi
    fi
...
if [ ! -f ./$VLVIEWER ] ; then
    if [[ "$OS" != "Windows_NT" && "$CENTOS_5" == "true" ]] ; then
        echoit "CentOS 5 is too old to support qt4.6; make vlviewer${POSTREBUILD}... SKIP"
    elif [ $MAKE != "" ] ; then
        echoit_n "Building vlviewer${POSTREBUILD}... "
